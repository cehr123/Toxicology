...
class MoleculeNet(Dataset):
    ...
    def __init__(
        ...
        max_len_smiles = {
            'herg': 188, 'ames': 175, 'dili': 174, 'ld50': 174,
            'skin_reaction': 98, 'carcinogens': 292, 'tox21': 325,
            'toxcast': 325, 'sider': 413, 'bbbp': 257, 'clintox': 323, 'bace': 194
        }
        ...
        self.conformer_dict = defaultdict(list)
        ...
        if os.path.exists(processed_dir):
            idx_name, z_name, pos_name, y_name, smiles_name, Q_name, tox_labels_name = [
            ...
            self.smiles_mm = np.memmap(
                smiles_name, mode="r", dtype=np.int64,
                shape=(self.y_mm.shape[0], self.max_len_smiles)
            )
            self.Q_mm = np.memmap(Q_name, mode="r", dtype=np.float32)
            self.tox_labels_mm = np.memmap(
                tox_labels_name, mode="r", dtype=np.float32,
                shape=(self.y_mm.shape[0], self.num_labels)
            )
            ...
            print(f"[INFO] Loaded preprocessed MMAP dataset from {processed_dir}")
            return
    ...
    def sample_iter(self, statistic_run=False):
        ...
        for i in tqdm(range(len(pickle_path_list))):
            ...
            if not statistic_run:
                ...
                self.conformer_dict[i].append(data)
                self.conformer_dict[i].append(slices)
    ...
    def process(self):
        ...
        num_all_confs = 0
        num_all_atoms = 0
        for data in self.sample_iter(statistic_run=True):
            num_all_confs += 1
        ...
        idx_name, z_name, pos_name, y_name, smiles_name, Q_name, tox_labels_name = self.processed_paths
        idx_mm = np.memmap(
        ...
        with open(f'{self.dataset}_multi_conf.pickle', 'wb') as f:
            pickle.dump(self.conformer_dict, f)
        self.conformer_dict = dict()
    ...
    def get(self, idx):
        ...
        import random
        multi_conf_path = f"{self.dataset}_multi_conf.pickle"
        if os.path.exists(multi_conf_path):
            with open(multi_conf_path, "rb") as f:
                conf_dict = pickle.load(f)
            data_list = conf_dict.get(idx, None)
            if data_list is not None and len(data_list) >= 2:
                data, slices = data_list[0], data_list[1]
                num_confs = len(data.y)
                rd_int = random.choice(range(num_confs))
                data = self.getter(data, slices, rd_int)
                print(f"[INFO] Randomly selected conformer {rd_int} out of {num_confs}")
                return data
            else:
                print(f"[WARN] No multi-conformer entry for index {idx}, falling back to single conformer mode.")
    ...
    def getter(self, data, slices, idx):
