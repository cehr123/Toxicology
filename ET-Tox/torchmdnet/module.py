    ...
    def multitask_auc(self, predicted, ground_truth, final_test=False):
        ...
        for i in range(n_tasks):
            if np.any(ground_truth_np[:, i] == 0) and np.any(
                ...
                valid_mask = ground_truth[:, i] >= 0
                if valid_mask.sum() > 0:
                    auc.append(auroc(predicted[valid_mask, i], ground_truth[valid_mask, i]))
                else:
                    auc.append(torch.tensor(0.5, device=predicted.device))  # neutral AUC if no valid labels
                auc_dict[i] = auc
            else:
                continue
